
export default {
  meta: [
    { charset: 'utf-8' },
    { name: 'viewport', content: 'width=device-width, maximum-scale=1.0, minimum-scale=1.0, initial-scale=1' },
    { rel: 'alternate', title: 'Atom Feed', type: 'application/atom+xml', href: 'http://hire.jonasgalvez.com.br/atom.xml' },
    { rel: 'alternate', title: 'RSS Feed', type: 'application/rss+xml', href: 'http://hire.jonasgalvez.com.br/rss.xml' },
  ],
  link: [
    { rel: 'stylesheet', href: 'https://fonts.googleapis.com/css?family=Old+Standard+TT' },
    { rel: 'stylesheet', href: 'https://code.cdn.mozilla.net/fonts/fira.css' },
    { rel: 'stylesheet', href: '/mobile.css' }
  ]
}
